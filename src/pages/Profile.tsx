...
import { CURRENCY, EXPLORER, SLABS_ENABLED } from '../config'
import { getCollection, getHistory, type OwnedCard, type HistoryCard } from '../slabs/client'
...
export default function Profile() {
  ...
  // Slabs (cards) data, only in the merged build and only for a connected wallet.
  const [cards, setCards] = useState<OwnedCard[] | null>(null)
  const [history, setHistory] = useState<HistoryCard[] | null>(null)
  useEffect(() => {
    if (!SLABS_ENABLED || !address) {
      setCards(null)
      setHistory(null)
      return
    }
    let cancelled = false
    void Promise.allSettled([getCollection(address), getHistory(address)]).then(([c, h]) => {
      if (cancelled) return
      setCards(c.status === 'fulfilled' ? c.value.cards : [])
      setHistory(h.status === 'fulfilled' ? h.value.cards : [])
    })
    return () => {
      cancelled = true
    }
  ...
  // Cards (Slabs) headline figures. insuredValueUsd is micro-dollars (1e6).
  const usd = (micro: string | null | undefined) => (micro ? Number(micro) / 1e6 : 0)
  const fmtUsd = (v: number) =>
    `$${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
  const cardsHeld = cards?.length ?? 0
  const insuredHeld = (cards ?? []).reduce((sum, c) => sum + usd(c.insuredValueUsd), 0)
  const bestPull = (cards ?? []).reduce<OwnedCard | null>(
    (best, c) => (usd(c.insuredValueUsd) > usd(best?.insuredValueUsd) ? c : best),
    null,
  )
  ...
  return (
    <section className="section profile">
      <div className="wrap">
        ...
        {/* Tournaments won — centered on its own row above the headline stats. */}
        <div className="profile__champion">
          <StatTile
        ...
        {/*
          Three panels, not two columns.

          Identity spans the full width and Teams / Battle history share the row beneath it,
          so the two lists start on the same line. They used to sit in two independent
          columns, which put Battle history level with the identity card and left the Teams
          panel floating halfway down beside it.
        */}
        <div className="profile__grid">
          {/* -------- identity -------- */}
          <div className="card profile__panel profile__identity">
        ...
        {/* -------- cards (Slabs) — merged build only -------- */}
        {SLABS_ENABLED && (
          <div className="card profile__panel profile__cards">
            <div className="profile__panelhead">
              <h3 className="profile__h3">Cards</h3>
              <Link className="mini-btn" to="/cards/collection">
                Collection
              </Link>
            </div>

            <div className="profile__cardstats">
              <div className="profile__cardstat">
                <div className="profile__cardstat-value">{cardsHeld}</div>
                <div className="profile__cardstat-label">Cards held</div>
              </div>
              <div className="profile__cardstat">
                <div className="profile__cardstat-value">{fmtUsd(insuredHeld)}</div>
                <div className="profile__cardstat-label">Insured value held</div>
              </div>
              <div className="profile__cardstat">
                <div className="profile__cardstat-value profile__cardstat-value--best">
                  {bestPull ? fmtUsd(usd(bestPull.insuredValueUsd)) : 'None yet'}
                </div>
                <div className="profile__cardstat-label">
                  {bestPull?.name ? bestPull.name : 'Best pull'}
                </div>
              </div>
            </div>

            {/* Centred while it is empty: a left-set heading with one short grey line under
                it, alone in a wide panel, reads as a list that failed to load. Once there
                are packs the heading goes back over its list, where it belongs. */}
            <div
              className={`profile__packhist${history !== null && history.length === 0 ? ' profile__packhist--empty' : ''}`}
            >
              <h4 className="profile__h4">Pack history</h4>
              {history === null ? (
                <Spinner label="Loading…" />
              ) : history.length === 0 ? (
                <p className="profile__hint">No packs opened yet.</p>
              ) : (
                <ul className="profile__packs">
                  {history.map((h) => (
                    <li key={h.orderId} className="pack">
                      <span className={`pack__tier pack__tier--${h.tier}`}>{h.tier}</span>
                      <span className="pack__name">{h.name ?? 'Card'}</span>
                      {h.grade && <span className="pack__grade">{h.grade}</span>}
                      <span className="pack__value">{fmtUsd(usd(h.insuredValueUsd))}</span>
                      <span className="profile__muted pack__date">
                        {new Date(h.openedAt).toLocaleDateString(undefined, {
                          day: 'numeric',
                          month: 'short',
                        })}
                      </span>
